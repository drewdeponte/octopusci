...
module Octopusci
  module Queue
    def self.enqueue(job_klass, proj_name, branch_name, github_payload, job_conf)
      owner = github_payload["repository"]["owner"]["name"]
      gh_pl_key = github_payload_key(owner, proj_name, branch_name)

      if job_pending?('octopusci:commit', owner, proj_name, branch_name)
        self.redis.set(gh_pl_key, Resque::encode(github_payload))

        job = Octopusci::JobStore.list_repo_branch(owner, proj_name, branch_name, 0, 1).first
        if job
    ...
    def self.job_pending?(queue, owner, proj_name, branch_name)
      size = Resque.size(queue)
      return [Resque.peek(queue, 0, size)].flatten.any? { |v| v["args"][0] == proj_name && v["args"][1] == branch_name }
    end
    
    def self.github_payload(owner, project_name, branch_name)
      Resque::decode(self.redis.get(github_payload_key(owner, project_name, branch_name)))
    end
    
    def self.github_payload_key(owner, proj_name, branch_name)
      "octopusci:github_payload:#{owner}:#{proj_name}:#{branch_name}"
    end
